...
class LazyAgentRegistry:
    ...
    def __getitem__(self, agent_name: str):
        """
        获取智能体 (懒加载)
        查缓存 -> 解析名字 -> 按路径动态导入模块 -> 找出 agent 类 -> 注入依赖 -> 实例化 -> 缓存 -> 返回
        """
        ...
        script_path = self._skill_map[skill_name] # 对应的 agent.py 路径
        ...
        try:
            ...
            """
            reflection, inspect
            这个场景里用反射，是因为系统在运行时只知道“我要某个 agent 名字”，但不知道具体类写死在哪，也不想把所有 agent 硬编码到主流程里。
            所以它需要动态完成这件事：
            - 找到对应的 agent.py
            - 看这个文件里定义了哪个 agent 类
            - 确认它是不是 AgentBase 子类
            ...
            这和 Spring 用反射扫描 Controller / Bean，其实思想上很像：
            - Spring 不想你每加一个 Controller 就去主程序里手写注册
            - 所以它扫描类、注解、方法，再自动挂 URL

            Spring 里反射常用于“自动发现 Controller/Bean”；这个项目里反射常用于“自动发现并加载某个 skill 里的 Agent 类”。
            """
