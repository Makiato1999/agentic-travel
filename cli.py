...
class AligoCLI:
    ...
    async def process_query(self, user_input: str):
        ...
        with self.console.status("思考中...", spinner="dots"):
            ...
            if long_term_summary:
                context_messages.append(Msg(name="system", content=long_term_summary, role="system")) # [长期总结]
            for msg in recent_context:
                context_messages.append(Msg(name=msg["role"], content=msg["content"], role=msg["role"])) # [最近几轮对话]
            context_messages.append(Msg(name="user", content=user_input, role="user")) # [用户刚刚这次输入]
            ...
            try:
                intention_result = await retry_with_backoff(
                    ...
                    # lambda 在这里就是把这个行为包装成一个可传递的函数参数, 重试时每次调用这个 lambda，都会重新生成一个新的协程
                    # 本质上就是把“值”和“产生值的行为”分开了
                    # 回调函数
            ...
            # retry_with_backoff() 里那个 raise 如果抛的是普通异常，就会被外层 except Exception as e 接住；
            # 只有抛的是 CircuitOpenError，才会走外层专门的 except CircuitOpenError。
            except CircuitOpenError:
                raise
    ...
    async def _get_long_term_summary(self, user_input: str = "") -> str:
        ...
        _get_long_term_summary() 你现在可以压成这三块：
        - preference
            用户长期偏好 / 背景信息
        - summary
            由 LLM 生成的长期历史摘要
            ...
            - 历史 trip
        - some trips
            根据当前 user_input 再补几条相关或兜底的具体历史行程
        """
        summary_parts = []
