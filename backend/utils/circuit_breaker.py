...
class CircuitBreaker:
    ...
    def __init__(
        ...
        # _failure_count 是 CircuitBreaker 实例级别的全局计数
        # 因为在 cli里 initialize_system() 里只创建了一个CircuitBreaker实例
    ...
    def record_failure(self) -> None:
        """
        记录一次失败调用
        - llm_resilience.retry_with_backoff 负责“这一次调用要不要重试、怎么重试”
        - circuit_breaker.record_failure()
        负责“这次完整调用最终失败了，我要不要把系统状态切到更保守”
        """
