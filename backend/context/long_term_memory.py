...
class LongTermMemory:
    ...
    def get_preference(self, pref_type: str = None) -> Any:
        ...
        Args:
            ...
            现在库里的preference的列表结构
            [
                {"type": "hotel_brands", "value": "万豪"},
                {"type": "airlines", "value": "国航"}
            ]
            可以把这个列表里的每一个对象，看成数据库里的一行。
            比如如果放到表里，可能长这样：
            | user_id | type          | value |
            |--------|---------------|-------|
            | 1      | hotel_brands  | 万豪  |
        ...
        Returns:
            为了使用层保留便利性，按当前项目使用方式看，preferences 存成 dict 会更自然
            库里的存 list 更像是为未来扩展/落关系数据库预留了结构
