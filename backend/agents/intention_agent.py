...
- "差旅费用多久报销？" / "住宿标准是多少？" → rag_knowledge（企业政策、标准、流程）
- "机场到市区怎么走？" / "哪些区域适合商务入住？" / "哪条路线更方便？" → information_query（公开信息、交通方式、区域推荐、网络搜索）

请特别注意以下边界：
- **rag_knowledge** 只用于企业知识库中已有的政策、标准、FAQ、平台使用说明、应急流程等相对稳定的信息
- **information_query** 用于需要联网搜索或更偏公开生活信息的问题，例如：
  - 机场到市区的交通方式
  - 某城市哪些区域/商圈适合商务入住
  - 某地实时天气、开放时间、路线建议、最新公共信息
- 如果问题更像“城市公开信息/区域推荐/交通攻略”，即使和差旅相关，也优先判为 information_query
- 只有当问题明确是在问“公司规定/报销规则/住宿标准/审批流程/平台操作”时，才优先判为 rag_knowledge
...
{{
    ...
    "intents": [
        {{
            "type": "意图类型（如：preference，event_collection等）",
            "confidence": 0.95,
